...
<script lang="ts">
  ...
  import { JtracListSearchInfo, JtracInfo, PermissionList } from '@/vo/uploadManager/index';
...
<Box column class="outter">
  ...
  <DataGrid id="upload-mgmt-Grid" columnDefs={uploadSveletColumn} {rowData} headerRows={2} onGridReady={onGridReadyHandler} pageShowFlag={false} />
  {#if rowData && rowData.length > 0}
    <Box horizontalAlign="compact" verticalAlign="middle" class="main-bottom" height="95px">
      <Text class="pageText">Total {rowData[0].totalCount} Item(s)</Text>
      <PaginationNav
        on:setPage={e => onPageChange(e)}
        totalItems={rowData[0].totalCount}
        pageSize={selectedPageSizeValue.code}
        {currentPage}
        limit={6}
